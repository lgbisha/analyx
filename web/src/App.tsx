...
import { useAuth, logout, LOGIN_URL } from "./auth-client";
...
const INFINI_APP_URL = "https://app.infinisynapse.cn";
...
const UI = {
  zh: {
    ...
    signupFoot: "免费注册",
    signupCard: "这份报告由 InfiniSynapse 引擎生成。注册即可用它分析你自己的数据，新用户可领免费体验额度。",
    signupBtn: "免费注册 →",
    signupErr: "也可以注册自己的 InfiniSynapse 账号直接体验 →",
    signupScenario: "用 InfiniSynapse 同款引擎分析你自己的数据，新用户注册可领免费体验额度 ·",
    ssoLogin: "使用 InfiniSynapse 登录",
    ssoLoginShort: "登录",
    ssoLoginTitle: "使用 InfiniSynapse 账号一键登录",
    ssoLogout: "退出",
    ssoUserTitle: "已通过 InfiniSynapse 登录",
  },
  en: {
    ...
    signupFoot: "Sign up free",
    signupCard: "This report was generated by the InfiniSynapse engine. Sign up to analyze your own data — new users get free credits.",
    signupBtn: "Sign up free →",
    signupErr: "Or sign up for your own InfiniSynapse account and try it yourself →",
    signupScenario: "Analyze your own data with the same InfiniSynapse engine — new users get free credits ·",
    ssoLogin: "Sign in with InfiniSynapse",
    ssoLoginShort: "Sign in",
    ssoLoginTitle: "One-click sign-in with your InfiniSynapse account",
    ssoLogout: "Sign out",
    ssoUserTitle: "Signed in with InfiniSynapse",
  },
};
...
export function App() {
  ...
  // 分享链接按当前访问域名动态拼（www / xishu 双域名并行，不写死）
  const shareUrl = sc && report ? `${window.location.origin}/s/${report.taskId}` : "";
  function copyShare() {
    if (!shareUrl) return;
    ...
    <div className={`app ${view === "home" ? "home-mode" : ""}`}>
      <nav className="nav">
        <div className="wrap navwrap">
          ...
          <div className="nav-right">
            ...
            {/* Partner SSO：凭据未配置（ssoEnabled=false）时整体不渲染，未登录一切照旧 */}
            {auth.ready && auth.ssoEnabled && !auth.user && (
              <a className="nav-login" href={LOGIN_URL} title={t.ssoLoginTitle}>
                <span className="nav-login-full">{t.ssoLogin}</span>
                <span className="nav-login-short">{t.ssoLoginShort}</span>
              </a>
            )}
            {auth.ready && auth.user && (
              <span className="nav-auth">
                <span className="nav-user" title={`${t.ssoUserTitle}: ${auth.user.nickname}`}>{auth.user.nickname}</span>
                <button className="nav-logout" onClick={() => logout()}>{t.ssoLogout}</button>
              </span>
            )}
      ...
      {view === "home" && (
        <>
          ...
          <footer className="foot">
            ...
            {" · "}
            <a href={INFINI_APP_URL} target="_blank" rel="noopener noreferrer">{t.signupFoot}</a>
          </footer>
        </>
      )}
      ...
      {view === "scenario" && sc && (
        <>
          ...
          <div className="wrap" style={{ ["--c" as any]: sc.brandColor }}>
            ...
            {mode === "dual_upload" && (
              <section className="block">
                ...
                <div className="dl-row" style={{ marginTop: 16 }}>
                  ...
                  {sc.exampleTaskId && <a className="link-btn" href={`/s/${sc.exampleTaskId}`} target="_blank" rel="noreferrer">{t.viewExample}</a>}
                </div>
                <div className="cta-block" style={{ border: "none", paddingTop: 20 }}>
            ...
            {/* 场景页注册引导（一行式，克制，不干扰主流程） */}
            <p className="signup-line">
              {t.signupScenario}{" "}
              <a href={INFINI_APP_URL} target="_blank" rel="noopener noreferrer">{t.signupBtn}</a>
            </p>
          </div>
        </>
      )}
      ...
      {view === "error" && (
        <div className="wrap">
          <section className="block error center">
            ...
            <p className="err-signup">
              <a href={INFINI_APP_URL} target="_blank" rel="noopener noreferrer">{t.signupErr}</a>
            </p>
          </section>
        </div>
      )}
      ...
      {view === "done" && report && (
        <div className="wrap">
          ...
          {/* 已通过 InfiniSynapse 登录的用户不再看注册引导（已是用户就不劝注册） */}
          {!auth.user && (
            <section className="signup-card">
              <p>{t.signupCard}</p>
              <a className="signup-pill" href={INFINI_APP_URL} target="_blank" rel="noopener noreferrer">{t.signupBtn}</a>
            </section>
          )}
        </div>
      )}
